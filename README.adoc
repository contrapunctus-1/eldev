...
get rebuilt and the files are (re)generated in process. Targets of the
second type always have names that begin with “:” (like keywords in
Elisp). The most important virtual target is called `:default` — this
is what Eldev will build if you don’t request anything explicitly.
...
Some projects, however, may decide to follow the advice of certain
linters, but not others. You can explicitly tell Eldev about a
project’s policy by adjusting one or more of the variables
`eldev-lint-default`, `eldev-lint-default-excluded` and
`eldev-lint-disabled` in file `Eldev`. All of these variables affect
exactly which linters Eldev starts when their names are not specified
explicitly.
