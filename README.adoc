...
* Eldev's configuration is Elisp.  The user can change many defaults
  or even define additional commands and options.
...
2. Now you can issue the following -
...
== How-to guides
=== XXX How to specify project files
...when they are not detected by the defaults.
// List some common cases of adding files or directories
...
=== How to exclude specific linters
...
=== How to generate autoloads
// Are these correct?

1. Enable the autoload plugin by adding `(eldev-use-plugin 'autoloads)` to the project’s `Eldev` file.
2. Run `$ eldev compile`

=== How to collect code coverage statistics
...
. Activate the plugin by adding `(eldev-use-plugin 'undercover)` to your
...
=== Running Eldev on different Emacs versions
...
In the future, plugins may become externally-managed and “detached”
from Eldev itself (create an issue if you are interested).  For now,
...
After the plugin is activated, no additional steps are needed to
instruct Eldev for using the generated file.  In fact, it is able to
